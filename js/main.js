...
async function getSearch(event) {
    try {
        ...
        let urlsearch = url + "/?name=" + search; //esto es un query param
...
{/* Copio aquí la estructura HTML para poderla tener de referencia al crearla en la función printOneCharactar() (justo debajo):
    ...
    divRow.innerHTML = `<figure class="col-md-4">
                        ...
                        <div class="col-md-8">
                            <div class="card-body">
                                ...
                                <p class="card-text">Estado: <span class="${pCharacter.status.toLowerCase()}">${pCharacter.status}</span></p>
                                <p class="card-text"><small class="text-body-secondary">${pCharacter.species}</small></p>
                                <a class="btn btn-sm btn-warning" href="character.html?id=${pCharacter.id}">Más info</a>
                            </div>
                        </div>`
...
async function init() {
    try {
        ...
        console.log(pErr); //esto es para el usuario hay que sacarlo con un alert o un notifications.
    }
